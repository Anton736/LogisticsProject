...
class ConstraintFactory:
    ...
    def _add_load_flow_constraints(self):
        ...
        for v in self.scenario.vehicles:
            # Определяем тип доставки (для будущего расширения)
            is_unit_delivery = (v.category.lower() == "штуки")

            # Лимит кузова зависит от типа машины
            ...
            for wh in self.scenario.warehouses:
                ...
                if outgoing_from_wh:
                    ...
                    for arc in outgoing_from_wh: self.model.add(is_starting_wh == 1).OnlyEnforceIf(arc)
                else:
                ...
                if is_unit_delivery:
                    total_pickup = sum(
                        self.var_manager.get_pickup_var(v.id, wh.id, b.id) for b in self.scenario.brands if
                        self.var_manager.get_pickup_var(v.id, wh.id, b.id) is not None)
                else:
                    total_pickup = sum(
                        self.var_manager.get_pickup_crates_var(v.id, wh.id, b.id) for b in self.scenario.brands if
                        self.var_manager.get_pickup_crates_var(v.id, wh.id, b.id) is not None)

                self.model.add(self.var_manager.get_load_at_point_var(v.id, wh.id) == total_pickup).OnlyEnforceIf(
                    is_starting_wh)

            # 2. ПЕРЕДАЧА ПОТОКА ПО МАРШРУТУ
                ...
                if isinstance(loc, Store):
                    for b in self.scenario.brands:
                        ...
                        if units_var is not None and crates_var is not None:
                            # Формула пересчета: 10 штук = 1 ящик, 41 штука = 2 ящика
                            self.model.add(crates_var * K >= units_var)
                            self.model.add(crates_var * K <= units_var + K - 1)

                            # Обнуляем, если не приехали
                            self.model.add(units_var == 0).OnlyEnforceIf(is_loc_visited.Not())
                            self.model.add(crates_var == 0).OnlyEnforceIf(is_loc_visited.Not())

                # Выбираем, в чем считать баланс машины
                if is_unit_delivery:
                    consumed = sum(
                        self.var_manager.get_delivery_var(v.id, loc.id, b.id) for b in self.scenario.brands if
                        self.var_manager.get_delivery_var(v.id, loc.id, b.id) is not None)
                    picked_up = sum(self.var_manager.get_pickup_var(v.id, loc.id, b.id) for b in self.scenario.brands if
                                    self.var_manager.get_pickup_var(v.id, loc.id, b.id) is not None)
                else:
                    consumed = sum(
                        self.var_manager.get_delivery_crates_var(v.id, loc.id, b.id) for b in self.scenario.brands if
                        self.var_manager.get_delivery_crates_var(v.id, loc.id, b.id) is not None)
                    picked_up = sum(
                        self.var_manager.get_pickup_crates_var(v.id, loc.id, b.id) for b in self.scenario.brands if
                        self.var_manager.get_pickup_crates_var(v.id, loc.id, b.id) is not None)

                # Закон сохранения
                self.model.add(
                    self.var_manager.get_load_at_point_var(v.id, loc.id) == self.var_manager.get_load_arriving_var(v.id,
                                                                                                                   loc.id) - consumed + picked_up).OnlyEnforceIf(
                    is_loc_visited)
                self.model.add(consumed <= self.var_manager.get_load_arriving_var(v.id, loc.id)).OnlyEnforceIf(
                    is_loc_visited)

                # Защита от перегруза
                self.model.add(self.var_manager.get_load_at_point_var(v.id, loc.id) <= max_capacity_val).OnlyEnforceIf(
                    is_loc_visited)

                # Машина пустая в конце маршрута (в последнем магазине)
                if isinstance(loc, Store):
                    is_end_here = self.var_manager.get_is_end_var(v.id, loc.id)
                    self.model.add(self.var_manager.get_load_at_point_var(v.id, loc.id) == 0).OnlyEnforceIf(is_end_here)
            ...
            for wh in self.scenario.warehouses:
                ...
                if incoming_to_wh:
                    ...
                    for arc in incoming_to_wh: self.model.add(is_ending_wh == 1).OnlyEnforceIf(arc)
                else:
    ...
    def _add_demand_satisfaction_constraints(self):
        for store in self.scenario.stores:
            ...
            # ЖЕСТКО: В каждый магазин обязана заехать хотя бы 1 машина (максимум 2)
            visits_to_this_store = [self.var_manager.get_is_visited_var(v.id, store.id) for v in self.scenario.vehicles
                                    if self.var_manager.get_is_visited_var(v.id, store.id) is not None]
            if visits_to_this_store:
                self.model.add(sum(visits_to_this_store) >= 1)
                self.model.add(sum(visits_to_this_store) <= 2)
            ...
            for brand in self.scenario.brands:
                ...
                if base_demand > 0:
                    # ГИБКО: План - это верхний потолок. Можно привезти меньше!
                    self.model.add(total_delivered <= base_demand)

                    # Защита от холостого пробега: если машина приехала, минимум 1 единицу она обязана отдать
                    for v in self.scenario.vehicles:
                        del_v = self.var_manager.get_delivery_var(v.id, store.id, brand.id)
                        vis_v = self.var_manager.get_is_visited_var(v.id, store.id)
                        if del_v is not None and vis_v is not None:
                            self.model.add(del_v >= 1).OnlyEnforceIf(vis_v)
                else:
                    self.model.add(total_delivered == 0)
