...
def build_route_map(routes_excel_path=r"data/маршруты_результат.xlsx",
    dict_excel_path='data/для логистики.xlsm',
    sheet_name='рабочее окно',
    ...
    df_routes = pd.read_excel(routes_excel_path)
    df_dict = pd.read_excel(dict_excel_path, sheet_name=sheet_name)

    # ==========================================
    # 2. БРОНЕБОЙНОЕ НАЗНАЧЕНИЕ КОЛОНОК
    # ==========================================
    COL_CODE_ROUTES = df_routes.columns[0]      # исходный код маршрута (строка с "06 маршрут...")
    COL_ROUTE_NUM = df_routes.columns[1]        # номер маршрута (тоже может быть строкой)
    COL_TIME = df_routes.columns[10]            # время
